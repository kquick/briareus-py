...
report(status_report(succeeded, project(ProjRepo), Strategy, Bldcfg, Vars)) :-
...
report(status_report(fixed, project(ProjRepo), Strategy, Bldcfg, Vars)) :-
    is_project_repo(ProjRepo),
    strategy(Strategy, ProjRepo, Branch),
    branch_type(BranchType, Branch),
    bldres(ProjRepo, BranchType, Branch, Strategy, Vars, Bldcfg, N, N, 0, 0, configValid),
...
report(status_report(initial_success, project(ProjRepo), Strategy, Bldcfg, Vars)) :-
    is_project_repo(ProjRepo),
    strategy(Strategy, ProjRepo, Branch),
    branch_type(BranchType, Branch),
    bldres(ProjRepo, BranchType, Branch, Strategy, Vars, Bldcfg, N, N, 0, 0, configValid),
...
report(status_report(failed, project(ProjRepo), Strategy, Bldcfg, Vars)) :-
    is_project_repo(ProjRepo),
    strategy(Strategy, ProjRepo, Branch),
    branch_type(BranchType, Branch),
...
report(var_failure(ProjRepo, N, V)) :-
    ...
    findall(X, (report(status_report(S,project(ProjRepo),_,X,Vars)),
                good_status(S),
