...
class RunCommand(ActorTypeDispatcher):
    ...
    def _start_command(self):
        ...
        if can_watch:
            ...
            self._log(command, "info", command.logtag + " watching %s and %s", self.p.stdout.fileno(), self.p.stderr.fileno())
            import fcntl
    ...
    def receiveMsg_WatchMessage(self, watchmsg, sender):
        subp = getattr(self, 'p', None)
        self._log(self.pending_commands[-1] if self.pending_commands else "no-cmd", "info", (self.pending_commands[-1].logtag if self.pending_commands else "no-logtag") + " watch %s and %s", watchmsg, subp)
        if subp:  # and self.pending_commands?
            ...
            if subp.poll() is not None:
                self._log(self.pending_commands[-1] if self.pending_commands else "no-cmd", "info", '>>>> poll not None, finishing')
