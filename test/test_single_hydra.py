...
def verify_primary_cfg_uncollated(bpath, bldcfgs):
    ...
    if other_bldcfgs:
        assert_eqlist(other_bldcfgs,
                      [bpath + '/hydra/_common/copy_hh.nix',
                       bpath + '/hydra/TheRepo/TheRepo-hydra-project-config.json',
                       'TheRepo.txt',
                      ])
    ...
    VerifyJobsetDesc(
        ...
        VerifyBldCfg('PR-frog.standard',
                     VerifyInput('TheRepo-src', 'git', 'https://github.com/frog_repo_url frog'),
                     VerifyInput('variant', 'string', 'PR', contains=True)),
        VerifyBldCfg('PR-toad.standard',
                     VerifyInput('TheRepo-src', 'git', 'https://github.com/toad_repo_url toad'),
                     VerifyInput('variant', 'string', 'PR', contains=True)),
        VerifyBldCfg('feat1.standard',
                     VerifyInput('TheRepo-src', 'git', 'https://github.com/the_repo_url feat1')),
        VerifyBldCfg('master.standard',
                     VerifyInput('TheRepo-src', 'git', 'https://github.com/the_repo_url master')),
        nixexprinput = 'TheRepo-src',
        nixexprpath = './release.nix',
    ).verify(bldcfgs[None])

    if other_bldcfgs:
        verify_cfg_instructions(bpath, bldcfgs['TheRepo.txt'])
        ...
        ).verify(json.loads(bldcfgs[bpath + '/hydra/TheRepo/TheRepo-hydra-project-config.json']))

        assert_in('cp ${hh_output} $out',
                  bldcfgs[bpath + '/hydra/_common/copy_hh.nix'])
...
def verify_primary_cfg_collated(bpath, bldcfgs):
    ...
    assert_eqlist([K for K in bldcfgs.keys() if K is not None],
                  [ bpath + '/hydra/TheRepo/TheRepo-hydra-project-config.json',
                    bpath + '/hydra/TheRepo/TheRepo-hydra-inputs-config.json',
                    bpath + '/hydra/_common/gen_inpupd.nix',
                    bpath + '/hydra/TheRepo/TheRepo_inpupd_jobsets.json',
                    bpath + '/hydra/TheRepo/TheRepo-updinputs.nix',
                    bpath + '/hydra/_common/gen_proj_jobsets.nix',
                    'TheRepo.txt',
                  ])
    ...
    ).verify(json.loads(bldcfgs[bpath + '/hydra/TheRepo/TheRepo-hydra-inputs-config.json']))
    ...
    VerifyJobsetDesc(
        VerifyBldCfg("update_inputs",
                     ...
                     VerifyInput('realize_inputs', 'path', bpath + '/hydra/TheRepo'),
                     # TheRepo-cfg-inp is KWQ? to re-run this if the input config changes??
                     VerifyInput('TheRepo-cfg-inp', 'path', bpath + '/fname'),
                     VerifyInput('TheRepo-PR91-src', 'git', 'https://github.com/frog_repo_url frog'),
                     VerifyInput('TheRepo-PR134-src', 'git', 'https://github.com/toad_repo_url toad'),
                     VerifyInput('TheRepo-feat1-src', 'git', 'https://github.com/the_repo_url feat1'),
                     VerifyInput('TheRepo-master-src', 'git', 'https://github.com/the_repo_url master'),
                     # nixpkgs is needed for the KWQ? expression
                     admin_nixpkgs,
        ),
        nixexprinput = 'realize_inputs',
        nixexprpath = 'TheRepo-updinputs.nix',
    ).verify(bldcfgs[bpath + '/hydra/TheRepo/TheRepo_inpupd_jobsets.json'])

    # The TheRepo_updinputs.nix is the main nix expression that is
    # associated with the update_inputs jobset.
    updinp_expr = bldcfgs[bpath + '/hydra/TheRepo/TheRepo-updinputs.nix']
